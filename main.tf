...
locals {
  instance_map = {
    for idx, name in var.instance_names : name => {
      id         = aws_instance.aap26vms[idx].id
      private_ip = aws_instance.aap26vms[idx].private_ip
    }
  }
}
...
resource "aws_route53_record" "controller" {
  ...
  type    = "A"
  ttl     = 300
  records = [local.instance_map["aap25controller"].private_ip]
}
...
resource "aws_route53_record" "gateway" {
  ...
  type    = "A"
  ttl     = 300
  records = [local.instance_map["aap25gateway"].private_ip]
}
...
resource "aws_route53_record" "pah" {
  ...
  type    = "A"
  ttl     = 300
  records = [local.instance_map["aap25pah"].private_ip]
}
...
resource "aws_route53_record" "eda" {
  ...
  type    = "A"
  ttl     = 300
  records = [local.instance_map["aap25eda"].private_ip]
}
...
resource "aws_route53_record" "pg" {
  ...
  type    = "A"
  ttl     = 300
  records = [local.instance_map["aap25pg"].private_ip]
}
...
resource "aws_route53_record" "en" {
  ...
  type    = "A"
  ttl     = 300
  records = [local.instance_map["aap25en"].private_ip]
}